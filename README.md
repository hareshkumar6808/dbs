...
The map uses category-specific, heading-aware aircraft silhouettes and renders aircraft individually at every zoom. Selecting a flight shows an exact-model, freely licensed Wikimedia Commons photograph with photographer and license attribution, an eight-fix planned airway route, the persisted flown track, and any operational route generated around active weather or restricted airspace. Sidebar totals describe the currently filtered map data; the daily-operations label identifies the full network total.
...
The demonstration network contains 40 Indian airports and 32 international airports across 27 countries, 18 airlines, 10 aircraft types, 760 aircraft, 274 routes, and 3,800 scheduled flight legs. International rotations run in both directions and connect foreign airports through Indian hubs; domestic services use Indian operators, and long-haul services use wide-body aircraft. The timetable is synthetic and approximates the scale and variety of India-linked traffic; it is not a live airline schedule.

Demo movement interpolates in **PostGIS** along the active operational geometry. Each map refresh calls a transaction-locked tick: all visitors share one ten-second time bucket, and `UNIQUE(flight_id, recorded_at)` prevents duplicate samples. Position rows are inserted before current aircraft state is updated. Speed and altitude vary through gate, taxi, climb, cruise, descent and arrival phases, while headings follow the local route tangent through gradual turns. Scheduled flights transition through `BOARDING`, `TAXIING`, `EN_ROUTE`, `APPROACHING`, and `LANDED`; deterministic fixtures also cover delayed, diverted and cancelled states. Exhausted aircraft receive fresh synthetic rotations with new flight IDs, preserving old history.
...
Map state uses a set-based join with indexed lateral latest-position lookups and aggregate factors. It returns up to 2,000 current flight records from a rolling operational window, while dashboard totals reflect the full 3,800-flight day. Optional `west`, `south`, `east`, and `north` parameters apply server-side viewport filtering; the client also removes offscreen markers from the DOM. Scenario examples provide a guaranteed flight for every operational state and active hazard and frame the relevant route on selection.

The map starts in a quiet operational view: the flight explorer and detailed filters are collapsed, weather and airspace overlays are opt-in, ground aircraft remain visible at airports, and the light-gray basemap keeps geography legible behind traffic. Light and dark themes persist between visits.
...
Map tiles use Esri's light gray canvas with visible attribution. Aircraft photos come from Wikimedia Commons and show their individual author and license links in the UI. The site uses IBM Plex fonts and Lucide icons; no proprietary flight-tracker assets are copied.
