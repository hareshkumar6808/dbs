...
Map state uses a set-based join with indexed lateral latest-position lookups and aggregate factors, rather than a separate query per flight. It returns up to 2,000 current flight records from a rolling operational window, not full tracks, while dashboard totals reflect the full 3,800-flight day. A larger deployment should use viewport paging and separate map/reference-data refreshes.
