...
export default function FlightMap({ state, flights, selected, onSelect, layers, alternates, history, replay, airport, reset, theme, scenarioFlightIds, showDisruptionImpacts }: { state: MapState; flights: Flight[]; selected?: Flight; onSelect: (id: number) => void; layers: Layers; alternates: Alternate[]; history: Position[]; replay?: Position; airport?: Airport; reset: number; theme: "dark" | "light"; scenarioFlightIds: number[]; showDisruptionImpacts: boolean }) {
  ...
  return <MapContainer center={[18, 70]} zoom={2} minZoom={2} maxZoom={13} maxBounds={[[-55, -170], [70, 180]]} maxBoundsViscosity={1} worldCopyJump={false} zoomControl={false} className={`flight-map theme-${theme}`}>
    <TileLayer noWrap subdomains="abcd" url="https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png" attribution="&copy; OpenStreetMap contributors &copy; CARTO" maxNativeZoom={20} /><ZoomControl position="bottomright" /><InitialFit flights={flights} /><Focus flight={selected} airport={airport} reset={reset} scenarioFlights={scenarioFlights} />
